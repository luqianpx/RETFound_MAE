...
def get_feature(model, device, args, ep):
    # feature extractor
    fea_extr = create_feature_extractor(model, {'fc_norm': 'fea1'})

    if not os.path.exists(args.task + args.dataset + '/'):
        os.makedirs(args.task + args.dataset + '/')
    ...
    model.eval()
    bn = 30
    for i in range(int(np.ceil(img_li.shape[0]/bn))):
        images = torch.tensor(img_li[i*bn:(i+1)*bn])
        images = images.to(device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast():
            output = model(images)
            prediction_softmax = nn.Softmax(dim=1)(output)
            fea = fea_extr(images)['fea1']

            pred_li.append(prediction_softmax.cpu().detach().numpy())
            fea_li.append(fea.cpu().detach().numpy())
    pred_li = np.concatenate(pred_li)
    fea_li = np.concatenate(fea_li)

    # save the result
    fi = args.task + args.dataset + '/fea_res_' + str(ep)
    with open(fi, 'wb') as fi:
        pickle.dump([na_li, lab_li, pred_li, fea_li], fi)
